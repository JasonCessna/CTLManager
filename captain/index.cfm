	...
	<cfswitch expression="#url.section#">
	 <!------------------------
		...
		<cfset weekLineup = getWeeklyLineup(session.token.access_token,getLineupRange)>
			...
			<cfform name="frmTemplate" method="POST" action="http://duttles.com/ctlmanager/captain/index.cfm?section=display&access=#hash(accessLevel)#">
				<div align="center">
		...
		------------------------->
		<cfdefaultcase>
			<cfform name="frmTemplate" method="POST" action="http://duttles.com/ctlmanager/captain/index.cfm&access=#hash(accessLevel)#">
			<div align="center">
				...
				<div>
					<input type="hidden" name="team" id="team" value="#hash(url.teamAccess)#">
